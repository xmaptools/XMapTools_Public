...
% XTHERMOTOOLS MATLAB code for XThermoTools.fig
%      XTHERMOTOOLS, by itself, creates a new XTHERMOTOOLS or raises the existing
%      singleton*.
%
%      H = XTHERMOTOOLS returns the handle to a new XTHERMOTOOLS or the handle to
%      the existing singleton*.
%
%      XTHERMOTOOLS('CALLBACK',hObject,eventData,handles,...) calls the local
%      function named CALLBACK in XTHERMOTOOLS.M with the given input arguments.
%
%      XTHERMOTOOLS('Property','Value',...) creates a new XTHERMOTOOLS or raises the
%      existing singleton*.  Starting from the left, property value pairs are
%      applied to the GUI before XThermoTools_OpeningFcn gets called.  An
%      unrecognized property name or invalid value makes property application
%      stop.  All inputs are passed to XThermoTools_OpeningFcn via varargin.
%
...
% Edit the above text to modify the response to help XThermoTools

% Last Modified by GUIDE v2.5 20-Aug-2021 08:25:30

% Begin initialization code - DO NOT EDIT
gui_Singleton = 1;
gui_State = struct('gui_Name',       mfilename, ...
                   'gui_Singleton',  gui_Singleton, ...
                   'gui_OpeningFcn', @XThermoTools_OpeningFcn, ...
                   'gui_OutputFcn',  @XThermoTools_OutputFcn, ...
                   'gui_LayoutFcn',  [] , ...
                   'gui_Callback',   []);
...
% #########################################################################
%       OPENING FUNCTION
function XThermoTools_OpeningFcn(hObject, eventdata, handles, varargin)
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% 
% Version
fid = fopen([LocBase,'/XTT_Local_version.txt']);
...
fid = fopen([LocBase,'/XTT_Dev/XTT_Def_ElDisp.txt']); Compt = 0;
while 1
    lalign = fgetl(fid);
...
fid = fopen([LocBase,'/XTT_Dev/XTT_Def_OxideName.txt']); Compt = 0;
while 1
    lalign = fgetl(fid);
    if isequal(lalign,'') || isequal(lalign,-1)
        break
...
Ratio = 0.6121;
if ScreenSize(3) > 1920       % introduced in 3.4.2
    ProportionScreen = 0.55;
else
...
function varargout = XThermoTools_OutputFcn(hObject, eventdata, handles) 
...
% --------------------------------------------------------------------
function TheriakDiagnostic_Callback(hObject, eventdata, handles)
%
Diagnostic_TheriakConnection;
return


% --------------------------------------------------------------------
function DownloadDLL_WIN_Callback(hObject, eventdata, handles)
% 

directoryname = uigetdir(cd, 'Select a directory');

h = waitbar(0,'The files are dowloading - Please wait...');
waitbar(0.5,h);

WebAdress = ['http://www.xmaptools.com/FTP_drive/Cygwindll4Theriak.zip'];
...
uiwait(msgbox({'The DLL files have been downloaded','You need to copy manually the DLL files to C:\Windows\ (4 files)'},'XMapTools','modal'));
