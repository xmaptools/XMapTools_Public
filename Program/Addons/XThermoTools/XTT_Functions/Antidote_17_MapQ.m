function [Output,Antidote_VARIABLES,handles] = Antidote_17_MapQ(AntidoteRecipes,AntidoteMode,WorkVariXMap,MinimOptions,handles)
%
%
...
% New PL - (03.03.2020) ** SWOT ** Store the modeled/observed compositions
MapData.MinComp.Min.ListEl = '';
MapData.MinComp.Min.CompMod = zeros(length(Pi),length(TCi));
MapData.MinComp.Min.CompXMap = zeros(length(Pi),length(TCi));
...
% New PL - (03.03.2020) ** SWOT ** Store the modeled/observed compositions
for j=1:length(Evaluation.MinComp_TEMP)
    ListMinComp{j}= Evaluation.MinComp_TEMP(j).MinName;
end

for i = 1:length(ListMinComp)
    WhereMin = find(ismember(MapData.MinComp.ListMin,ListMinComp{i}));
    
    if WhereMin > length(MapData.MinComp.Min)
        MapData.MinComp.Min(WhereMin).ListEl = '';   % Add a new mineral
    end
    
    for j = 1:length(Evaluation.MinComp_TEMP(i).MinEl)
        Ind3 = find(ismember(MapData.MinComp.Min(WhereMin).ListEl,Evaluation.MinComp_TEMP(i).MinEl{j}));
        if Ind3
            WhereSave = Ind3;
        else
            WhereSave = length(MapData.MinComp.Min(WhereMin).ListEl)+1;
            MapData.MinComp.Min(WhereMin).ListEl{WhereSave} = char(Evaluation.MinComp_TEMP(i).MinEl{j});
        end
        
        MapData.MinComp.Min(WhereMin).CompMod(iP,iTC,WhereSave) = Evaluation.MinComp_TEMP(i).MinCompMod(j);
        MapData.MinComp.Min(WhereMin).CompXMap(iP,iTC,WhereSave) = Evaluation.MinComp_TEMP(i).MinCompXMap(j);
        MapData.MinComp.Min(WhereMin).Unc1s(iP,iTC,WhereSave) = Evaluation.MinComp_TEMP(i).MinUnc1s(j);
    end
...
PlotChemPotMap = 0;
PlotModelDeviation = 1;

 
DateStr = char(datestr(now));
DateStr(find(DateStr == ' ')) = '_';
ProjectName = ['Antidote_Recipe17_',DateStr];
...
if PlotModelDeviation
    for i = 1:length(MapData.MinComp.ListMin)
         ListElem4Plot = MapData.MinComp.Min(i).ListEl;
         for j=1:length(ListElem4Plot)
             h = figure; 
             Data2Plot = MapData.MinComp.Min(i).CompMod(:,:,j)-MapData.MinComp.Min(i).CompXMap(:,:,j);
             imagesc(TCi,Pi,Data2Plot), colorbar
             set(gca,'YDir','normal')
             CmapAdj = RdYlBu(65);
             CmapAdj(33,:) = [1,1,1];
             colormap([CmapAdj])
             caxis([-max(abs(Data2Plot(:))),max(abs(Data2Plot(:)))])
             xlabel('Temperature (C)')
             ylabel('Pressure (kbar)')
             title(['Calc-Obs for ',MapData.MinComp.Min(i).ListEl{j},'-in-',MapData.MinComp.ListMin{i},' (apfu)'])
             ElN = MapData.MinComp.Min(i).ListEl{j};
             if isequal(ElN(end),'*')
                 ElN = ElN(1:end-1);
             end
             saveas(gcf,[Directory,'7_Comparison_',MapData.MinComp.ListMin{i},'_',ElN,'.fig'],'fig');
             close(h)
         end
    end
...
if PlotChemPotMap
    for i = 1:length(MapData.ChemPot.ListChemPot)
        ...
        saveas(gcf,[Directory,'8_MU',char(MapData.ChemPot.ListChemPot{i}),'.pdf'],'pdf');
        saveas(gcf,[Directory,'8_MU_',char(MapData.ChemPot.ListChemPot{i}),'.fig'],'fig');
