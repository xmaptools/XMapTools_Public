function [Output,Antidote_VARIABLES] = Antidote_11a12_Wind(Mode,WorkVariXMap,MinimOptions,app)
%
%
%
...
switch Mode
    case 'Growth'
        app.Report_Antidote{end+1} = ['Antidote: Recipe [12] - Growing window (find optimal P–T, variable bulk)'];
    case 'Scan'
...
% NbXi = 3;
% NbYi = 4;
% for iLoop = 1:NbXi*NbYi
%     [iX,iY] = CalcIndices(app,iLoop,NbXi,NbYi);
%     disp([num2str(iLoop),': ',num2str(iX),' ',num2str(iY)])
% end
% keyboard
...
%for iX = 1:length(Xi)
    %for iY = 1:length(Yi)
        ...
        app.Report_Antidote{end+1} = [' -->  LOOP: ',num2str(Compt),'/',num2str(NbLoops)];
        app.Report_Antidote{end+1} = '';
        app.Text_Report_Antidote.Value = app.Report_Antidote;
        pause(0.1)
...
switch Mode
    case 'Growth'
        app.Report_Antidote{end+1} = ['               SUMMARY (Growing Window):'];
    case 'Scan'
...
% function [iX,iY] = CalcIndices(iLoop,NbXi,NbYi)
% 
% if iLoop <= NbYi
%     iY = iLoop;
%     iX = 1;
